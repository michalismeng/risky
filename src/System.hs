...
cpuHardware initialProg initialMem = output
    where
        ...
        write = mux writeEnable (Just <$> bundle ((unpack . resize) <$> readAddr', writeValue)) (pure Nothing)

        memData = firstCycleDef $ blockRamPow2 initialMem ((unpack . resize) <$> readAddr') write

        (regFile, next_pc, readAddr, writeValue, writeEnable) = pipeline instruction memData
...
topEntity clk rst = withClockReset clk rst $ cpuHardware (ld_forward_ICache ++ repeat 0) (ld_forward_DCache ++ repeat 0)
